...
class ImapClient:
    ...
    # 建立IMAP SSL连接，根据SSL设置决定使用IMAP4_SSL或IMAP4连接方式
    ...
    async def search_uids(self, criteria: str) -> list[str]:
        ...
        try:
            ...
            # 连接异常，尝试重连并重试一次
            logger.warning("IMAP connection aborted during SEARCH, reconnecting and retrying... criteria=%s, error=%s",
                           criteria, e)
    ...
    async def fetch_rfc822(self, uid: str) -> Message:
        ...
        try:
            ...
            # 连接异常，尝试重连并重试一次
            logger.warning("IMAP connection aborted during FETCH, reconnecting and retrying... uid=%s, error=%s", uid,
                           e)
            await self._reconnect()
            # 重连后重试
            return await asyncio.to_thread(_fetch)
